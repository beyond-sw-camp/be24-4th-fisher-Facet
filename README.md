...
## 🖥️ 6. Facet 블루 그린 무중단 배포 테스트

---

## 💡 블루-그린을 선택한 3가지 핵심 이유

### 1. 실시간 통신 규격의 일치 (WebSocket Integrity)

**카나리의 위험성**
- 트래픽의 일부(예: 10%)만 새 버전으로 라우팅
- 소켓 통신 규격 변경 시, 구버전/신버전 사용자 간 **데이터 형태 불일치** 또는 **통신 에러** 발생 가능

**블루-그린의 강점**
- 특정 시점에 **모든 유저를 Green 환경으로 일괄 전환**
- 전 사용자가 동일한 소켓 규격과 UI를 사용 → 버전 혼재로 인한 이상 현상 원천 차단

---

### 2. 실시간 경매의 형평성 (Auction Fairness)

**데이터 동기화 문제**
- 카나리 배포 중 구/신 서버 간 미세한 **DB 처리 속도 차이** 또는 **로직 차이** 발생
- 경매 입찰 시간·낙찰가에 대한 **분쟁 가능성**

**블루-그린의 해결**
- 모든 유저가 **동일한 로직을 가진 서버 그룹** 내에서 경쟁
- 경매 시스템의 핵심 가치인 **'형평성'** 완전 보장

---

### 3. 즉각적인 롤백 (Instant Rollback)

**리스크 관리**
- 실시간 금융·경매 거래 서비스 특성상, 배포 직후 치명적 버그 발견 시 **즉시 이전 상태로 복구** 필요

**블루-그린의 효율성**
```
배포 후 버그 발견
       ↓
로드밸런서 타겟 전환 (Blue로 복귀)
       ↓
⚡ 수 초 내 완전 복구
```
- Blue(이전 버전) 환경을 **그대로 유지**하기 때문에 언제든 안전하게 복구 가능
- 데이터 사고를 방지하기 위한 **최선의 선택**
...
<div style="text-align: center;">
  <img src="https://github.com/beyond-sw-camp/be24-4th-fisher-Facet/raw/main/docs/%EB%AC%B4%EC%A4%91%EB%8B%A8%EB%B0%B0%ED%8F%AC%ED%85%8C%EC%8A%A4%ED%8A%B8/%E1%84%91%E1%85%B3%E1%84%85%E1%85%A9%E1%86%AB%E1%84%90%E1%85%B3%E1%84%86%E1%85%AE%E1%84%8C%E1%85%AE%E1%86%BC%E1%84%83%E1%85%A1%E1%86%AB%E1%84%87%E1%85%A2%E1%84%91%E1%85%A9.gif" width="100%" alt="프론트엔드 무중단 배포 시연">
</div>
...
<div style="text-align: center;">
  <img src="docs/무중단배포테스트/백엔드무중단배포테스트.gif" width="100%" alt="백엔드무중단 배포 시연">
</div>
